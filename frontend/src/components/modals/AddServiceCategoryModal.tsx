...
import { servicesApi, ServiceCategory } from '../../services/api/servicesApi';
...
export function AddServiceCategoryModal({ isOpen, onClose, onSuccess, editData }: AddServiceCategoryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      if (editData) {
        await servicesApi.updateCategory(editData.id, formData);
        toast.success('Kategori güncellendi');
      } else {
        await servicesApi.createCategory(formData);
                ...
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                    <Dialog.Title as="h3" className="text-base font-semibold leading-6 text-gray-900 dark:text-white">
                      {editData ? 'Kategori Düzenle' : 'Yeni Kategori Ekle'}
                    </Dialog.Title>
                    <div className="mt-2">
