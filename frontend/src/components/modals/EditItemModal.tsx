...
export function EditItemModal({ isOpen, onClose, onSuccess, item }: EditItemModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.patch(`/items/${item.id}`, formData);
      onSuccess();
      onClose();
    } catch (err: any) {
      setError(err.response?.data?.message || 'Eşya güncellenirken bir hata oluştu');
    } finally {
      setLoading(false);
    }
  };
                ...
                >
                  ...
                  {rooms.map((room) => (
                    <option key={room.id} value={room.id}>
                      {room.room_number} - {room.warehouse?.name}
                    </option>
                  ))}
                </select>
              </div>
