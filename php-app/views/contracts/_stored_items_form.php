...
<div class="<?= $storedItemsFormCompact ? '' : 'border-t border-gray-200 dark:border-gray-600 pt-4 mt-2' ?>">
    ...
    <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">Her eşya için durum seçin: Sıfır, Koli, Kullanılmış veya Hasarlı.</p>
    <?php endif; ?>
            ...
            <tbody id="contractItemsBody">
                <?php if (!empty($items)): ?>
                    <?php foreach ($items as $i => $item): ?>
                        <?php $rowNum = $i + 1; require __DIR__ . '/_contract_item_row.php'; ?>
                    <?php endforeach; ?>
                <?php else: ?>
                    <?php $item = []; $rowNum = 1; require __DIR__ . '/_contract_item_row.php'; ?>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
...
(function() {
    ...
    var itemConditionOptions = <?= json_encode(itemConditionOptions(), JSON_UNESCAPED_UNICODE) ?>;
    var selectClass = 'w-full min-w-[110px] px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm dark:bg-gray-700 dark:text-white';
    function buildConditionSelect(selected) {
        selected = selected || 'sifir';
        var html = '<select name="item_condition[]" required class="' + selectClass + '">';
        Object.keys(itemConditionOptions).forEach(function(code) {
            html += '<option value="' + code + '"' + (selected === code ? ' selected' : '') + '>' + itemConditionOptions[code] + '</option>';
        });
        return html + '</select>';
    }
    window.buildContractItemRowHtml = function(n) {
        return '<td class="px-3 py-2 text-sm text-gray-600 dark:text-gray-400">' + n + '</td>' +
            '<td class="px-3 py-2"><input type="text" name="item_name[]" placeholder="Örn: Koltuk takımı" class="w-full min-w-[120px] px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm dark:bg-gray-700 dark:text-white"></td>' +
            '<td class="px-3 py-2">' + buildConditionSelect('sifir') + '</td>' +
            ...
            '<td class="px-3 py-2"><input type="text" name="item_description[]" placeholder="Renk, boyut vb." class="w-full min-w-[100px] px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm dark:bg-gray-700 dark:text-white"></td>' +
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('#addContractItemRow')) {
            ...
            tr.innerHTML = window.buildContractItemRowHtml(n);
