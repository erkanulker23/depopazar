...
<div class="mb-6">
    ...
    <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-1">Teklif Düzenle</h1>
    <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-widest font-bold"><?= htmlspecialchars($title) ?></p>
    <div class="mt-2 flex flex-wrap gap-2">
        <a href="/teklifler/<?= htmlspecialchars($id) ?>/yazdir" target="_blank" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl border border-gray-300 dark:border-gray-600 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
...
<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm mobile-card overflow-visible md:overflow-hidden">
    <form method="post" action="/teklifler/guncelle" class="p-6">
        ...
        <div class="space-y-6">
                ...
                <div class="flex flex-wrap gap-4">
                    <?php foreach ($proposalTypeLabels as $val => $l): ?>
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="proposal_type" value="<?= htmlspecialchars($val) ?>" <?= $proposalType === $val ? 'checked' : '' ?> class="proposal-type-radio rounded-full border-gray-300 text-emerald-600 focus:ring-emerald-500">
            ...
            <div id="proposalDepoSection" class="border-t border-gray-200 dark:border-gray-700 pt-4 <?= $proposalType !== 'depo' ? 'hidden' : '' ?>">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">Bu teklif hangi depo için geçerli?</p>
                <div class="max-w-md">
                    ...
                    <select name="warehouse_id" id="warehouse_id" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-emerald-500 dark:bg-gray-700 dark:text-white">
                        ...
                        <?php foreach ($warehouses as $w): ?>
                            <option value="<?= htmlspecialchars($w['id']) ?>" <?= ($warehouseId && $warehouseId === ($w['id'] ?? '')) ? 'selected' : '' ?>><?= htmlspecialchars($w['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <div id="proposalNakliyeSection" class="border-t border-gray-200 dark:border-gray-700 pt-4 <?= $proposalType === 'depo' ? 'hidden' : '' ?>">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea name="pickup_address" rows="3" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-emerald-500 dark:bg-gray-700 dark:text-white"><?= htmlspecialchars($pickupAddress) ?></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Eşya / Hizmet Teslim Adresi (Açık Adres)</label>
                        <textarea name="delivery_address" rows="3" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-emerald-500 dark:bg-gray-700 dark:text-white"><?= htmlspecialchars($deliveryAddress) ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="title" value="<?= htmlspecialchars($title) ?>" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-emerald-500 dark:bg-gray-700 dark:text-white">
                </div>
                <div>
                    ...
                    <table class="min-w-full border border-gray-200 dark:border-gray-600 rounded-xl" id="itemsTable">
                        ...
                        <tbody id="itemsBody">
                            ...
                            <tr class="item-row">
                                ...
                                <td class="px-3 py-2">
                                    <input type="text" name="item_name[]" value="<?= htmlspecialchars($item['name'] ?? '') ?>" class="w-full px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm dark:bg-gray-700 dark:text-white">
                                    <input type="hidden" name="item_service_id[]" value="<?= htmlspecialchars($item['service_id'] ?? '') ?>">
                                </td>
                                <td class="px-3 py-2"><input type="text" name="item_quantity[]" value="<?= htmlspecialchars($item['quantity'] ?? '1') ?>" class="w-20 px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm dark:bg-gray-700 dark:text-white"></td>
                                <td class="px-3 py-2"><input type="text" name="item_unit_price[]" value="<?= number_format((float)($item['unit_price'] ?? 0), 2, ',', '') ?>" class="w-24 px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm dark:bg-gray-700 dark:text-white"></td>
                                <td class="px-3 py-2"><input type="text" name="item_description[]" value="<?= htmlspecialchars($item['description'] ?? '') ?>" class="w-full px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm dark:bg-gray-700 dark:text-white"></td>
                            ...
                            <tr class="item-row">
                                ...
                                <td class="px-3 py-2"><button type="button" class="remove-item text-red-600 hover:text-red-700 p-1"><i class="bi bi-trash"></i></button></td>
                            </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="total_amount" id="total_amount" value="<?= number_format($totalAmount, 2, ',', '') ?>" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-emerald-500 dark:bg-gray-700 dark:text-white">
                    </div>
                    <div>
                ...
                <div class="mt-4">
                    ...
                    <textarea name="notes" rows="4" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-emerald-500 dark:bg-gray-700 dark:text-white"><?= htmlspecialchars($notes) ?></textarea>
                </div>
            </div>
        </div>
...
<script>
(function() {
    var form = document.querySelector('form[action="/teklifler/guncelle"]');
